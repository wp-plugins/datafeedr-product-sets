...
if ( ! class_exists( 'Dfrps_Tools' ) ) {
	...
	class Dfrps_Tools {
		...
		function admin_menu() {
			add_submenu_page(
				'dfrps',
				__( 'Tools &#8212; Datafeedr Product Sets', DFRPS_DOMAIN ), 
				__( 'Tools', DFRPS_DOMAIN ), 
				'manage_options', 
				$this->key,
				array( $this, 'output' ) 
			);
		...
		function output() {
			...
			jQuery(function($) {
				$('#dfrps_test_loopbacks').on('click',function(e) {
					...
					$("#dfrps_test_loopbacks").text('<?php _e("Testing...", DFRPS_DOMAIN); ?>').addClass('button-disabled');
					$.ajax({
						type: "POST",
				...
				$('#dfrps_start_batch_image_import').on('click',function(e) {
					...
					$.ajax({
						...
						$("#dfrps_stop_batch_image_import").show().html('<?php _e("Stop", DFRPS_DOMAIN); ?>');
						reload_batch_import();
					});
					e.preventDefault();
				});
				...
				$('#dfrps_stop_batch_image_import').on('click',function(e) {
					...
					$.ajax({
						...
						$("#dfrps_stop_batch_image_import").text('<?php _e("Stop", DFRPS_DOMAIN); ?>').removeClass('button-disabled').hide();
					});
					e.preventDefault();
				});
		...
		function section_batch_image_import_desc() { ?>
			<p>
				...
				<?php _e( 'If you want to download product images before they are displayed on your website, click the <strong>[Start Image Import]</strong> button to begin importing images for products which do not already have an image.', DFRPS_DOMAIN ); ?><br />
				<span class="dfrps_warning"><?php _e( 'This process is server intensive. Use with caution!', DFRPS_DOMAIN ); ?>
			</p>
			<p>
				<a href="#" id="dfrps_start_batch_image_import" class="button"><?php _e("Start Image Import", DFRPS_DOMAIN); ?></a> 
				<a href="#" id="dfrps_stop_batch_image_import" class="button" style="display: none"><?php _e("Stop", DFRPS_DOMAIN); ?></a>
			</p>
			<div id="dfrps_start_batch_image_import_result" style="padding: 10px; border: 1px solid silver; display: none; background: #FFF;"></div>
		<?php
		}
